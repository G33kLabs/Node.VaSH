...
(function( $ ){
	...
	var methods = {
		...
		init : function( options ) {
			...
			settings.api_url = 'https://apis.google.com/js/plusone.js' ; 
			methods.installJS() ;
		...
		triggerAction: function(action, message) {
			$.gplus.log(action+' => '+JSON.stringify(message)) ;
		...
		installJS: function() {
			...
			if ( match ) {
				$.gplus.log('|-> G+ loader is already Loaded...') ;
				_s.onComplete() ;
			} else {
				$.gplus.log('|-> Install G+ loader JS API...') ;
				(function() {
					$.gplus.log('|-> Load API File...') ;
	...
	// PLugin Call
	$.fn.gplus = function( method ) {
		if ( methods[method] ) {
			return methods[method].apply( this, Array.prototype.slice.call( arguments, 1 ));
		} else if ( typeof method === 'object' || ! method ) {
			return methods.init.apply( this, arguments );
		} else {
			$.error( 'Method ' +  method + ' does not exist on jQuery.gplus' );
		}    
	};
	
	$.gplus = $(document).gplus ;
	
	// Log function
	$.gplus.log = function(txt) {
    	try {
    		console.log('google+ API: '+JSON.stringify(txt));
	    } catch(e) { }
	} ;
